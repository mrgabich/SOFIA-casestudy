        ...
        case "$WORKLOAD_TYPE" in
                WORKLOAD_BAREMETAL)
                        ...
                        case "$ARCHITECTURE" in
                                ...
                                ARM_CORTEX_A75) export CPU_VARIANT=Cortex-A75MPx1; export armType=arm; export ENVIRONMENT=ovparmv8; export armSemihost=armAngel; CMD_OVP="$CMD_OVP --override enableVFPAtReset=on";;
                                ARM_CORTEX_AARCH64) export CPU_VARIANT=AArch64; export armType=arm; export ENVIRONMENT=ovparmv8; export armSemihost=armAngel; CMD_OVP="$CMD_OVP --override enableVFPAtReset=on";;
				...
				[[ "$ARCHITECTURE" = 'SIFIVEX280' ]] ; then
                        ...
                        export MAKEFILE_CFLAGS="-g -w -I$SUPPORT_FOLDER -I$OVP_FIM -D$ENVIRONMENT -DBAREMETAL -fcommon -O$OPTFLAG"
...
do
        ...
        if [[ "$ONLY_GOLD" -eq 1 ]]; then
            ...
            if [[ "$COUNTER" -eq "$VISITED_FOLDERS" ]]; then
                ...
                COUNTER=$((COUNTER+1))
                if [[ $(uptime | awk '{n=split($0, array, " ")} END {print $(n-2)}' | cut -d "." -f 1) -ge 5 ]]; then
